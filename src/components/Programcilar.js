...
export default function Programcilar() {
  ...
  const [oneCikanİd,setOneCikanİd] = useState(null);
  ...
  const oneCikanProgramcilar = programcilar.find((dev) => dev.id === oneCikanİd)
  return oneCikanProgramcilar.isim 
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color:oneCikanİd ? "gold" : 'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          enIyilerListesi.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setOneCikanİd(dev.id) /* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
